...
export function Modal({ isOpen, onClose, title, children }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div
        className="relative bg-[#111827] border-t border-slate-700/60 rounded-t-2xl flex flex-col"
        style={{ maxHeight: "92vh", minHeight: "40vh" }}
      >
        {/* Drag handle */}
        ...
        {/* Scrollable body */}
        <div
          className="overflow-y-auto px-5 py-5 flex-1"
          style={{ overscrollBehavior: "contain", WebkitOverflowScrolling: "touch" } as React.CSSProperties}
        >
          {children}
          {/* Bottom padding so last element clears keyboard on mobile */}
          <div className="h-8" />
        </div>
      </div>
    </div>
