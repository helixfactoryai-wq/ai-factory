...
import { ArrowLeft, Pencil, Trash2, GitBranch, Server, Calendar, Clock, Globe, Tag, Info, CheckCircle2, Code2, Lightbulb, FlaskConical, Wand2, ChevronRight, Cpu } from "lucide-react";
    ...
    <div className="space-y-5 animate-in">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setEditOpen(true)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-[#1E2939] border border-slate-700/60 text-slate-400">
            <Pencil className="w-4 h-4" />
          </button>
          <button onClick={() => setDeleteOpen(true)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-red-500/10 border border-red-500/20 text-red-400">
      ...
      <button onClick={() => navigate("/projects/" + project.id + "/prompts")}
        className="w-full card p-4 flex items-center gap-4 active:bg-[#1E2939] transition-colors text-left">
        <div className="w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center flex-shrink-0">
          <Wand2 className="w-5 h-5 text-indigo-400" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-slate-200">Prompt Builder</p>
          <p className="text-xs text-slate-500 mt-0.5">
            {promptCount > 0 ? promptCount + " prompt" + (promptCount !== 1 ? "s" : "") : "No prompts yet — tap to create one"}
          </p>
        </div>
